...
class Bms_chart_of_accounts extends CI_Controller {
    ...
    function __construct () { 
        ...
        $this->load->model('bms_coa_model');  
    }

     
    public function coa_acc_list ($offset=0, $per_page = 25) {
        $data['browser_title'] = 'Property Butler | Chart Of Account Name';
        $data['page_header'] = '<i class="fa fa-code"></i> Chart Of Account Name';
        ...
        $this->load->view('finance/chart_of_account/coa_name_list',$data);
	}
    
    
   
    public function get_coa_acc_list() {        
        ...
        if (isset($_POST['offset']) && is_numeric($_POST['offset']) && $_POST['offset'] >= 0 && isset($_POST['rows']) && is_numeric($_POST['rows']) && $_POST['rows'] >= 0 && $_POST['rows'] <= 100) {
            ...
            $list_arr = $this->bms_coa_model->get_coa_name_list ($_POST['offset'], $_POST['rows'], $search_txt);
        }      
                
        echo json_encode($list_arr);
	}
    
    function coa_acc_form ($charge_code_id = '')  {
        ...
        $data['properties'] = $this->bms_masters_model->getMyProperties ();
        $data['cat_group'] = $this->bms_coa_model->getChargeCodeCatGroup ();
        if($charge_code_id != '') {
            $data['charge_code_info'] = $this->bms_coa_model->get_charge_code_details($charge_code_id);
            if(empty($data['charge_code_info'])) {
                redirect('index.php/bms_dashboard/index'); 
            }
            $data['charge_code_sub_cat_info'] = $this->bms_coa_model->get_charge_code_sub_cat_details($charge_code_id);
    ...
    function charge_code_form_submit () {
        ...
        if(isset($charge_code['charge_code_category_name']) && trim($charge_code['charge_code_category_name']) !='') {
            ...
            if($charge_code_id) {
                $this->bms_coa_model->update_charge_code($charge_code,$charge_code_id);
                $type = 'edit';                
            } else {
                $charge_code_id = $this->bms_coa_model ->insert_charge_code($charge_code);                
            }
                ...
                foreach($sub_cat['charge_code_sub_category_name'] as $skey=>$sval) {
                    ...
                    if($sval != '') {
                        ...
                        if(!empty($sub_cat['charge_code_sub_category_id'][$skey])) {
                            $this->bms_coa_model->update_charge_code_sub_cat($sub_cat_data,$sub_cat['charge_code_sub_category_id'][$skey]);    
                        } else {
                            $this->bms_coa_model->insert_charge_code_sub_cat($sub_cat_data);
                        }                             
                    }
                }
            }                                   

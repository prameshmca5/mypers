...
class Bms_attendance extends CI_Controller {
    ...
    function get_staff_attendance () {
        ...
        $header .= '<table id="example2" class="table table-bordered table-hover" '.(isset($act_type) && $act_type == 'download_excel' ? 'border="1"' : '').'>
                <thead>
                <tr>
                  <th>Date</th>
                  <th>Staff Name</th>
                ...
                for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
                                ...
                                if($val['in_out_type'] == '1') {
                                    ...
                                    if(isset($val['atten_time']) && $val['atten_time'] != '') {
                                        ...
                                        if(isset($val['img_name']) && $val['img_name'] != '' && isset($act_type) && $act_type != 'download_excel')
                                            $remarks .= '<a href="javascript:;" class="atten_img" data-date="'.date('m.d.Y', strtotime($val['attn_date'])).'" data-value="'.$val['img_name'].'" data-att-type="Other In" data-att-time="'.date('d-m-Y', strtotime($val['attn_date'])).' '.date('h:i A', strtotime($val['atten_time'])).'" > ';
                                    ...
                                    if(isset($val['atten_time']) && $val['atten_time'] != '') {
                                        ...
                                        $remarks .= '<b>OOT:</b>';
                                        if(isset($val['img_name']) && $val['img_name'] != '' && isset($act_type) && $act_type != 'download_excel')
                                            $remarks .= '<a href="javascript:;" class="atten_img" data-date="'.date('m.d.Y', strtotime($val['attn_date'])).'" data-value="'.$val['img_name'].'" data-att-type="Other Out" data-att-time="'.date('d-m-Y', strtotime($val['attn_date'])).' '.date('h:i A', strtotime($val['atten_time'])).'" > ';
                                        $remarks .= date('h:i A', strtotime($val['atten_time']));
                                        if(isset($val['img_name']) && $val['img_name'] != '' && isset($act_type) && $act_type != 'download_excel')
                                            $remarks .='</a> ';
                                        $remarks .= ($val['remarks'] != '' ? ' ('.$val['remarks'] .') ' : '');                                       
                        ...
                        if($staff_name != '' && $design != '') {
                            ...
                            if(isset($act_type) && $act_type != 'download_excel') {
                                ...
                                if($in_img != ''){
                                    $in_img = '<a href="javascript:;" class="atten_img" data-date="'.date('m.d.Y', strtotime($atten_date)).'" data-value="'.$in_img.'" data-att-type="In Time" data-att-time="'.date('d-m-Y', strtotime($atten_date)).' '.date('h:i A', strtotime($intime)).'" > Sign In </a>';                    
                                }
                                if($out_img != '') {
                                    $out_img = '<a href="javascript:;" class="atten_img" data-date="'.date('m.d.Y', strtotime($atten_date)).'" data-value="'.$out_img.'" data-att-type="Out Time" data-att-time="'.date('d-m-Y', strtotime($atten_date)).' '.date('h:i A', strtotime($outtime)).'">Sign Out</a>';
                                } 
    ...
    function get_staff_absence () {
        ...
        $str .= '<table id="example2" class="table table-bordered table-hover" '.(isset($act_type) && $act_type == 'download_excel' ? 'border="1"' : '').'>
                <thead>
                <tr>

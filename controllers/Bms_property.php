...
class Bms_property extends CI_Controller {
    ...
    function add_property_submit () {
        ...
        if(isset($_POST) && !empty($_POST['property'])) {
            ...
            $property['tax_effect_from'] = !empty($property['tax_effect_from']) ? date('Y-m-d',strtotime($property['tax_effect_from'])) : '0000-00-00';
            $property['mf_rounding'] = !empty($property['mf_rounding']) ? $property['mf_rounding'] : '0';
            $property['sf_rounding'] = !empty($property['sf_rounding']) ? $property['sf_rounding'] : '0';
            $property['lpi_rounding'] = !empty($property['lpi_rounding']) ? $property['lpi_rounding'] : '0';
            ...
            if(empty($property['electricity'])) {
                ...
                $property['electricity_min_charg'] = '';
                $property['electricity_charge_per_unit'] = '';
            }
            if(empty($property['water'])) {
                $property['water'] = '0';
                $property['water_min_charg'] = '';
                $property['water_charge_per_unit'] = '';
            }
            ...
            if(!empty($property['tot_sq_feet'])) {
                $property['tot_sq_feet'] = floatval(preg_replace('/[^\d.]/', '', $property['tot_sq_feet']));
            }
            if(!empty($property['per_sq_feet'])) {
                $property['per_sq_feet'] = floatval(preg_replace('/[^\d.]/', '', $property['per_sq_feet']));
            }
            
            if(!empty($property['tot_share_unit'])) {
                $property['tot_share_unit'] = floatval(preg_replace('/[^\d.]/', '', $property['tot_share_unit']));
            }
            if(!empty($property['per_share_unit'])) {
                $property['per_share_unit'] = floatval(preg_replace('/[^\d.]/', '', $property['per_share_unit']));
            }
            ...
            // track for charges value changes
            $prop_hidd = $this->input->post('prop_hidd');
            if(!empty($prop_hidd)) {
                $hist_info = array();
                $hist_info['property_id'] = $property_id;
                $hist_info['updated_date'] = date('Y-m-d');
                $hist_info['updaded_by'] = $_SESSION['bms']['staff_id'];
                if($prop_hidd['calcul_base'] != '' && $prop_hidd['calcul_base'] != $property['calcul_base']) {
                    $hist_info['charg_cat'] = 1;
                    $hist_info['charg_val'] = $prop_hidd['calcul_base'];
                    $this->bms_property_model->insert_property_charg_hist($hist_info);
                }
                if($prop_hidd['sinking_fund'] != '' && $property['sinking_fund'] != '' && $prop_hidd['sinking_fund'] != $property['sinking_fund']) {
                    $hist_info['charg_cat'] = 2;
                    $hist_info['charg_val'] = $prop_hidd['sinking_fund'];
                    $this->bms_property_model->insert_property_charg_hist($hist_info);
                }
                if($prop_hidd['per_sq_feet'] != '' && !empty($property['per_sq_feet']) && $prop_hidd['per_sq_feet'] != $property['per_sq_feet']) {
                    $hist_info['charg_cat'] = 3;
                    $hist_info['charg_val'] = $prop_hidd['per_sq_feet'];
                    $this->bms_property_model->insert_property_charg_hist($hist_info);
                }
                if($prop_hidd['per_share_unit'] != '' && !empty($property['per_share_unit']) && $prop_hidd['per_share_unit'] != $property['per_share_unit']) {
                    $hist_info['charg_cat'] = 4;
                    $hist_info['charg_val'] = $prop_hidd['per_share_unit'];
                    $this->bms_property_model->insert_property_charg_hist($hist_info);
                }
                if($prop_hidd['monthly_billing'] != '' && !empty($property['monthly_billing']) && $prop_hidd['monthly_billing'] != $property['monthly_billing']) {
                    $hist_info['charg_cat'] = 5;
                    $hist_info['charg_val'] = $prop_hidd['monthly_billing'];
                    $this->bms_property_model->insert_property_charg_hist($hist_info);
                }
                // for insurance premium & quit rent
                unset($hist_info['charg_cat']); unset($hist_info['charg_val']);
                if(($prop_hidd['insurance_prem'] != '' && $property['insurance_prem'] != '' && $prop_hidd['insurance_prem'] != $property['insurance_prem']) 
                    || ($prop_hidd['insur_prem_date'] != '' && $property['insur_prem_date'] != '' && strtotime($prop_hidd['insur_prem_date']) != strtotime($property['insur_prem_date']))) {
                    $hist_info['prem_quit_cat'] = 1;
                    $hist_info['prem_quit_val'] = !empty($prop_hidd['insurance_prem']) ? $prop_hidd['insurance_prem'] : '';
                    $hist_info['prem_quit_effect_date'] = !empty($prop_hidd['insur_prem_date']) ? $prop_hidd['insur_prem_date'] : '';
                    $this->bms_property_model->insert_property_prem_quit_hist($hist_info);
                }
                
                if(($prop_hidd['quit_rent'] != '' && $property['quit_rent'] != '' && $prop_hidd['quit_rent'] != $property['quit_rent']) 
                    || ($prop_hidd['quit_rent_paid_on'] != '' && $property['quit_rent_paid_on'] != '' && strtotime($prop_hidd['quit_rent_paid_on']) != strtotime($property['quit_rent_paid_on']))) {
                    $hist_info['prem_quit_cat'] = 2;
                    $hist_info['prem_quit_val'] = !empty($prop_hidd['quit_rent']) ? $prop_hidd['quit_rent'] : '';
                    $hist_info['prem_quit_effect_date'] = !empty($prop_hidd['quit_rent_paid_on']) ? $prop_hidd['quit_rent_paid_on'] : '';
                    $this->bms_property_model->insert_property_prem_quit_hist($hist_info);
                }
            }
    ...
    function add_resi_notice_submit () {
        ...
        if(isset($resi_notice['property_id']) && trim($resi_notice['notice_title']) !='') {
            ...
            if(!empty($resi_notice['resident_notice_id']))
                ...
                $this->bms_property_model->insert_resident_notice($resi_notice); 
    ...
    function set_facility () {
        ...
        //$_SESSION['flash_msg'] = 'Facility Saved successfully!';

              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_unit_setup/add_unit');?>'" value="Add Unit" type="button">
                    </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_unit_setup/get_unit_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.floor_no+'</td>';
                    str += '<td>'+item.owner_name+'</td>';
                    str += '<td>'+item.unit_status+'</td>';                    
                    str += '<td style="text-align: center;">';
                    //str += '<a href="<?php echo base_url('index.php/bms_unit_setup/unit_details');?>/'+item.unit_id+'" title="View"><i class="fa fa-info-circle"></i></a> &ensp;';
                    str += '<a href="<?php echo base_url('index.php/bms_unit_setup/add_unit');?>/'+item.unit_id+'" title="Edit"><i class="fa fa-edit"></i></a>';
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_unit_setup/unit_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

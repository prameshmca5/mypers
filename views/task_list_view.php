    ...
    $.ajax({
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                ...
                var next_link = "";
                if(offset > 0 ){
                    previous_link = "<a href='javascript:;' onclick='loadOverSeeingTask("+(eval(offset)-eval(rows))+","+rows+",false);'><span class='glyphicon glyphicon-triangle-left' style='color: green;'></span></a> ";
                } 
                
                if(eval(numFound) > (eval(offset)+eval(rows))){
                    if((eval(offset)+eval(rows)) < numFound){
                        next_link = "<a href='javascript:;' onclick='loadOverSeeingTask("+(eval(offset)+eval(rows))+","+rows+",false);'> <span class='glyphicon glyphicon-triangle-right' style='color: green;'></span></a>";
                    } else {
                       // do nothing      
                    }                    
                }

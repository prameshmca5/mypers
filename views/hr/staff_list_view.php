              ...
              <div class="box-body">
                  <div class="row">
                    <div class="col-md-6 col-xs-9">
                          ...
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Staff">
                    ...
                    <div class="col-md-5 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_human_resource/staff_profile');?>'" value="Add Staff" type="button">
                    </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_human_resource/get_staff_list');?>',
        data: {'offset':offset,'rows':rows,'search_txt':search_txt},
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.desi_name+'</td>';
                    str += '<td>'+item.emp_type_name+'</td>';                    
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_human_resource/staff_profile');?>/'+item.staff_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_human_resource/staff_list');?>/'+offset+'/'+rows+'?search_txt='+search_txt;

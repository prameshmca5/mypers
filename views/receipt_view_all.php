...
?>      
            ...
            <form method="POST">
              ...
              <button type="button" class="btn btn-primary" style="" onclick="location.href = '<?php echo base_url();?>index.php/Receipt/receipt_list_view/';">New Recipt</button>
 ...
 if(empty($block)){ $blo="" ; } else {  $blo="and payment.block_id="."'$block'" ;}
if(empty($reciptno)){ $recipte="" ; } else {  $recipte="and payment.receiptno="."'$reciptno'" ;}
if(empty($unitno)){ $unit="" ; } else { $unit="and payment.unit_no="."'$unitno'" ;}
if(empty($sdate)){ $date="" ; } else {  $date="and payment.paydate BETWEEN " . "'$snewDate'" .  " and " ."'$enewDate'";}
if(empty($year)){ $years="" ; } else { $years="and payment.year="."'$year'" ;}
...
<table id="example2" align="center" class="table table-bordered table-hover" style="border: 1px solid #999;border-radius: 2px;width: 96%;">
 ...
 //echo "SELECT * FROM  payment join payment_detail on payment.receiptno = payment_detail.receiptno where payment.selstatus='0' $recipte $blo $unit $date $years $banks $checknos $froms $cancles $returns $issue $pay  $categorys $subcategorys $unitnosds $names order by payment.payment_id desc group by payment.receiptno";
 $quer= $this->db->query("SELECT * FROM  payment join payment_detail on payment.receiptno = payment_detail.receiptno where payment.selstatus='0' $recipte $blo $unit $date $years $banks $checknos $froms $cancles $returns $issue $pay  $categorys $subcategorys $unitnosds $names group by payment.receiptno order by payment.payment_id desc");
   $codd = $quer->num_rows();
   if($codd > 0){
  ...
  { ?>
					...
					$query = $this->db->query("SELECT unit_no FROM bms_property_units where unit_id = '$rowe->unit_no'");
						...
						?>
							...
							<td class="cntr"><?php echo $unitno = $row->unit_no;?></td>
							<td class="cntr"><?php    $querydp = $this->db->query("SELECT owner_name FROM  bms_property_units where unit_id='$rowe->unit_no'");
  foreach ($querydp->result() as $rowpp)
  { echo $ownner = $rowpp->owner_name; }?></td>
							...
							<a role="button" class="btn btn-primary" data-toggle="modal" data-target="#myModals<?php echo $rr = $rowe->receiptno; ?>" >Email</a>
							
<a href="<?php echo base_url();?>index.php/receipt/edit_reciept/<?php echo $rowe->receiptno;?>"  class="btn btn-primary btn-Edit"><i class="fa fa-edit"></i></a>
&nbsp;&nbsp;
<a href="<?php echo base_url();?>index.php/receipt/delete_reciept/<?php echo $rowe->receiptno;?>" onclick="return confirm('Are you sure you want to delete this reciepts?');" class="btn btn-danger btn-Remove"><i class="fa fa-trash"></i></a> </td>
        ...
        <div class="modal-body">
         ...
         <div class="row">
            ...
            <div class="form-group">
              ...
              <div class="col-md-8" style="color:red;">
               <?php  $uuid = $rowe->unit_no;?> <?php echo $qua ;?>
              </div>
              </div>
            </div>
            </div><hr>
            ...
            $querydp = $this->db->query("SELECT * FROM  bms_property_units where unit_id='$uuid' LIMIT 1");

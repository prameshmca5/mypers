                            ...
                            <div class="col-md-12 col-xs-12" style="padding-top: 5px;">
                                ...
                                <table style="width: 100%;" class="table table-bordered table-hover  table-striped">
                                        <tr>
                                            ...
                                            <th style='padding: 10px; border:1px solid #ddd;'>Adjustment Amt</th>
                                            <th style='padding: 10px; border:1px solid #ddd;'>Balance Amt</th>                                             
                                        </tr>
                                        <?php 
                                        $total = 0;
                                        if(!empty($credit_note_items)) {
                                            foreach($credit_note_items as $key=>$val) {
                                                ...
                                                echo "<td style='padding: 10px; border:1px solid #ddd;' align='right'>".(!empty($val['item_amount']) ? number_format($val['item_amount'],2) : '-')."</td>";
                                                echo "<td style='padding: 10px; border:1px solid #ddd;' align='right'>".(!empty($val['adj_amount']) ? number_format($val['adj_amount'],2) : '-')."</td>";
                                                echo "<td style='padding: 10px; border:1px solid #ddd;' align='right'>".(!empty($val['bal_amount']) ? number_format($val['bal_amount'],2) : '-')."</td>";
                                                echo "</tr>";
                                                $total += !empty($val['adj_amount']) ? $val['adj_amount'] : 0;
                                            }
                                            if($total > 0) {
                                                echo "<tr><td colspan='4' align='right' style='padding: 10px; border:1px solid #ddd;'> <b>Total Adjustment Amt</b> </td>";
                                                echo "<td align='right' style='padding: 10px; border:1px solid #ddd;'>".number_format($total,2)." </td>";
                                                echo "<td align='right' style='padding: 10px; border:1px solid #ddd;'>&nbsp;</td></tr>";
                                            }
                                        } else {
                                            echo "<tr><td colspan='6' align='center' style='padding: 10px; border:1px solid #ddd;'> No Record Found! </td></tr>";
                                        } ?>

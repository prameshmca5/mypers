...
function get_unit_receipts (unit_id) {
    ...
    $.ajax({
        ...
        success: function(data) {  
            ...
            if(data.length > 0) {                    
                $.each(data,function (i, item) {
                    str += '<option value="'+item.receipt_id+'" data-total-amount="'+item.paid_amount+'" >'+item.receipt_no+' ('+item.paid_amount+')'+'</option>';
                });
            } 
...
$('#add_line_item').click (function () {
    ...
    row += '<input type="hidden" name="items[debit_note_item_id][]" id="debit_note_item_id_'+id+'" value=""  />';
    ...
    row += '</div>';
    row += '<div class="col-md-2">';
    row += '<input type="number" name="items[item_amount][]" class="form-control amt_cal" value="" >';
    row += '</div>';
...
function remove_item (id){
    if($('#debit_note_item_id_'+id).val() != '') {
        if(confirm('You cannot undo this action. Are you sure want to delete?')) {
            $.ajax({
                type:"post",
                async: true,
                url: '<?php echo base_url('index.php/bms_fin_debit_note/unset_debit_note_item');?>',
                data: {'debit_note_item_id':$('#debit_note_item_id_'+id).val()},

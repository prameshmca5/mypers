            ...
            success: function(data) {
                ...
                if(numFound > 0) {
                    $.each(data.records,function (i, item) {
                        ...
                        //str += '<td> <a href="javascript:;" class="pay_det_cls" data-value="22" title="View">'+item.pay_no+'</a></td>';
                        str += '<td>'+datachecked(item.pay_no)+'</td>';
                        if(item.pay_service_provider_id==0){
                            str += '<td>-</td>';
                            str += '<td>'+item.pay_service_provider_name+'</td>';
                        }else {
				 ...
				 $('.pay_det_cls').unbind('click');
				$('.pay_det_cls').bind("click",function () {
					 $("#content_tbody").LoadingOverlay("show"); 
					$('.modal-body2').load('<?php echo base_url('index.php/bms_fin_payment/payment_popup_view/');?>'+$(this).attr('data-value'),function(result){

              ...
              <div class="box-body">
                  <div class="row">   
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Purchase Order">
                    </div>
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Purchase Order" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_purchase_order/purchase_add');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_purchase_order/get_purchase_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td><a href="javascript:;" class="purchase_det_cls" data-value="'+item.pur_order_id+'" data-value2="26"  title="View">'+item.po_no+'</a></td>';
                    str += '<td>'+item.provider_name+'</td>';                    
                    str += '<td>'+item.person_incharge+'</td>';
                    str += '<td>'+(item.DATE != '' ? formatDate(item.DATE) : '')+'</td>';
                    str += '<td>'+(item.delivery_date != '' ? formatDate(item.delivery_date) : '')+'</td>';
                    str += '<td>'+item.total+'</td>';  
                    str += '<td>';
                                var paystatus = "";
                                if(item.invoice_create_status == '0'){
                                   // if(item.invoice_paid_amount!=item.total){
                                     //   str += '<span class="label label-warning large" style="padding: 8px;">PARTLY INVOICED</span>';
                                    //}else {
                                        str += '<span class="label label-danger large" style="padding: 8px;">NOT INVOICED</span>';
                                    //}
                                }else {
                                    str += '<span class="label label-success large" style="padding: 8px;">INVOICED</span>';
                                }
                    str +=   '</td>';
                    //str += '<td>'+(item.invoice_create_status == 0 ? '<span class="label label-danger large" style="padding: 8px;">NOT INVOICED</span>' : '<span class="label label-success large clickable" style="padding: 8px;">INVOICED</span>')+'</td>';
                    str += '<td style="text-align: center;">';
                    str += '<a href="javascript:;" class="purchase_det_cls" data-value="'+item.pur_order_id+'" data-value2="26"  title="View"><i class="fa fa-info-circle"></i></a> &ensp;';
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1') {
                        str += '<a href="<?php echo base_url('index.php/bms_fin_purchase_order/purchase_add');?>/' + item.pur_order_id + '" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                        str += '<a href="javascript:;" data-id="' + item.pur_order_id + '" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });
			...
			$('.purchase_det_cls').unbind('click');
            $('.purchase_det_cls').bind("click",function () {
                $('.modal-body2').load('<?php echo base_url('index.php/bms_fin_purchase_order/purchase_popup/');?>'+$(this).attr('data-value'),function(result){
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_fin_purchase_order/get_purchase_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;
...
function remove_item (id){
    ...
    if(confirm('You cannot undo this action. Are you sure want to delete?')) {
        $.ajax({
            type:"post",
            async: true,
            url: '<?php echo base_url('index.php/bms_fin_purchase_order/unset_bill');?>',
            data: {'pur_id':id},
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                ...
                <h4 class="modal-title modeltitledisp"><i class="fa fa-file"></i> Purchase Order</h4>
            </div>
            ...
            <div class="modal-footer">
				...
				<button type="button" class="btn btn-default" href="javascript:;" onclick="printDiv('<?php echo base_url('index.php/bms_fin_purchase_order/purchase_popup/');?>')">Print</button>

              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-2 col-xs-12" style="padding-top: 25px;">
                        <input class="btn btn-primary add_btn" value="Add Receipt" type="button">
                    </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_receipt/get_receipt_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td><a href="<?php echo base_url().'index.php/bms_fin_receipt/receipt_details/';?>'+item.receipt_id+'">'+item.receipt_no+'</a></td>';
                    ...
                    str += '<td>'+item.paid_amount+'</td>';  
                                  
                    str += '<td style="text-align: left;">';
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_receipt/receipt_details');?>/'+item.receipt_id+'/download\')" title="Download"><i class="fa fa-download"></i></a>&ensp;';
                    str += '<a href="javascript:;" onclick="printDiv(\'<?php echo base_url('index.php/bms_fin_receipt/receipt_details');?>/'+item.receipt_id+'/print\')" title="Print"><i class="fa fa-print"></i></a>&ensp;';
                    if(parseFloat(item.open_credit) > 0) {
                        str += '<a href="<?php echo base_url('index.php/bms_fin_receipt/add_receipt');?>/'+item.receipt_id+'/amend" title="Amend"><i class="fa fa-pencil"></i></a>&ensp;';
                    }
                    
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1')  { 
                        //str += '<a href="<?php echo base_url('index.php/bms_fin_receipt/add_receipt');?>/'+item.bill_id+'" title="Edit"><i class="fa fa-info"></i></a>&ensp;';
                        str += '<a href="<?php echo base_url('index.php/bms_fin_receipt/add_receipt');?>/'+item.receipt_id+'" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                        str += '<a href="javascript:;" data-id="'+item.receipt_id+'" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_fin_receipt/receipt_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

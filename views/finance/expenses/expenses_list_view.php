              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Expense Invoice" type="button">
                    </div>
                </div>
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_fin_expenses/add_expenses');?>?property_id='+$("#property_id").val();
        return false;
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_fin_expenses/get_expenses_list');?>',
        ...
        success: function(data) {
            ...
            if(numFound > 0) {
                $.each(data.records,function (i, item) {
                    ...
                    str += '<td><a href="javascript:;" class="expen_det_cls" data-value="'+item.exp_inv_id+'" data-value2="26"  title="View">'+item.exp_doc_no+'</a></td>';
                    str += '<td>'+datachecked(item.po_no)+'</td>';
                    str += '<td>'+item.provider_name+'</td>';
                    str += '<td>'+item.person_incharge+'</td>';
                    str += '<td>'+(item.exp_date != '' ? formatDate(item.exp_date) : '')+'</td>';
                    ...
                    str += '<td>';
                                ...
                                if(item.payment_status == '0'){
                                    if(item.payment_paid_amount!='0.00' && item.payment_paid_amount!=item.total){
                                        str += '<span class="label label-warning large" style="padding: 8px;">PARTLY PAID</span>';
                                    }else {
                                        str += '<span class="label label-danger large" style="padding: 8px;">UNPAID</span>';
                                    }
                                }else {
                                    str += '<span class="label label-success large" style="padding: 8px;">PAID</span>';
                                }
                    str +=   '</td>';
                    str += '<td style="text-align: center;">';
                    str += '<a href="javascript:;" class="expen_det_cls" data-value="'+item.exp_inv_id+'" title="View"><i class="fa fa-info-circle"></i></a> &ensp;';
                    if('<?php echo in_array($_SESSION['bms']['designation_id'],$this->config->item('accounts_edit_del_desi')) ? 1 : 0;?>' == '1') {
                        str += '<a href="<?php echo base_url('index.php/bms_fin_expenses/add_expenses');?>/' + item.exp_inv_id + '" title="Edit"><i class="fa fa-edit"></i></a>&ensp;';
                        str += '<a href="javascript:;" data-id="' + item.exp_inv_id + '" class="del_cls" title="Delete" style="color:red"><i class="fa fa-close"></i></a>';
                    }
                    str += '</td>';
                    str += '</tr>';
                });

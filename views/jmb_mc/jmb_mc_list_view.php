            ...
            ?>
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search JMB / MC">
                    </div>
                    
                    
                    <div class="col-md-1 col-xs-1" >
                        <a href="javascript:;" role="button" class="btn btn-primary jmb_mc_filter"><i class="fa fa-search"></i></a>
                    </div>
                    
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_jmb_mc/add_jmb_mc');?>'" value="Add JMB / MC" type="button">
                    </div>
              ...
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <th>Unit No</th>
                  <th>Elected Date</th>
                  <th class="col-md-2">JMB/MC Role</th>
                  <th>Email</th>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_jmb_mc/get_jmb_mc_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.jmb_desi_name+'</td>';
                    str += '<td>'+item.unit_no+'</td>';
                    str += '<td>'+formatDate(item.elect_date)+'</td>';
                    str += '<td>'+item.jmb_role+'</td>';      
                    str += '<td>'+item.email_addr+'</td>';              
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_jmb_mc/add_jmb_mc');?>/'+item.member_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            $('.showing_stat').html('Showing '+(eval(offset)+(numFound > 0 ? 1 : 0))+' - '+showing_to+' Of '+numFound+' Record(s).'); // Total jmb_mc(s) : '+$('#property_id').find("option:selected").attr('data-value')
            //$(this).attr('title',$('#property_id').find("option:selected").attr('data-value'));
            
            // This is to update the url
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_jmb_mc/jmb_mc_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

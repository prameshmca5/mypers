              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Meeting">
                    </div>
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_meetings/add_meeting');?>'" value="Add Meeting" type="button">
                    </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary">
        ...
        <h4 class="modal-title">Meeting Checklist Update</h4>
      </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_meetings/get_meetings_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.meeting_descrip+'</td>';
                    str += '<td>'+item.meeting_date+'</td>';
                    str += '<td>'+item.meeting_time+'</td>';
                    str += '<td>'+item.meeting_venue+'</td>';
                    //str += '<td>'+item.unit_status+'</td>';                    
                    str += '<td style="text-align: center;">';
                    str += '<a class="meeting_chklist_update_cls" href="javascript:;" data-value="'+item.meeting_id+'" title="Checklist View & Update"><i class="fa fa-info-circle"></i></a> &ensp;';
                    str += '<a href="<?php echo base_url('index.php/bms_meetings/meeting_minutes');?>/'+item.meeting_id+'" title="Minutes of Meeting"><i class="fa fa-edit"></i></a> &ensp;';
                    str += '<a href="<?php echo base_url('index.php/bms_meetings/add_meeting');?>/'+item.meeting_id+'" title="Edit"><i class="fa fa-pencil"></i></a>';
                    str += '</td>';
                    str += '</tr>';
                });
                ...
                $('.meeting_chklist_update_cls').unbind("click");
                $('.meeting_chklist_update_cls').bind("click",function () {
                    $('.modal-content').css('width','750px');
                    $('.modal-body2').load('<?php echo base_url('index.php/bms_meetings/meetings_checklist_update/');?>'+$(this).attr('data-value'),function(result){
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_meetings/meetings_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

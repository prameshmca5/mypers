              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Vendor" type="button">
                    </div>
...
<script>
var service_provider_cat = $.parseJSON('<?php echo json_encode($this->config->item('service_provider_cat'))?>');
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_home_butler/vendor_form');?>';
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_home_butler/get_vendor_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.vendor_name+'</td>';                    
                    str += '<td>'+item.vendor_cat_name+'</td>';
                    str += '<td>'+item.vendor_incharge+'</td>';
                    str += '<td>'+item.vendor_mobile_no+'</td>';
                    str += '<td>'+item.vendor_inc_email+'</td>';
                                                         
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_home_butler/vendor_form');?>/'+item.vendor_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });

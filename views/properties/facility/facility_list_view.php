    ...
    <section class="content container-fluid cust-container-fluid">
      ...
      <!--------------------------
              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_btn" value="Add Facility" type="button">
                    </div>
        ...
        <table id="example2" class="table table-bordered table-hover">
            <thead>
            <tr>
              <th class="hidden-xs col-md-1">S No</th>
              <th class="col-md-6">Name</th>
              <th class="col-md-3">Deposit Require</th>           
...
$(document).ready(function () {
    ...
    $('.add_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_property/add_facility');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_property/get_facility');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.facility_name+'</td>';                    
                    
                    str += '<td>'+(item.deposit_require == '0' ? ' No ' : ' Yes ')+'</td>';
                                                         
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_property/add_facility');?>/'+item.facility_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_property/facility_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

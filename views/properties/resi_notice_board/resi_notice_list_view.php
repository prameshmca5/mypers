              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary" onclick="window.location.href='<?php echo base_url('index.php/bms_property/add_resi_notice');?>'" value="Add Notice" type="button">
                    </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary">
        ...
        <h4 class="modal-title">Resident Notice Details</h4>
      </div>
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_property/get_resi_notice_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.notice_title+'</td>';
                    str += '<td>'+item.start_date+'</td>';
                    str += '<td>'+item.end_date+'</td>';
                                     
                    str += '<td style="text-align: center;">';
                    str += '<a href="javascript:;" class="notice_det_cls" data-value="'+item.resident_notice_id+'" title="View"><i class="fa fa-info-circle"></i></a>&ensp;';
                    str += '<a href="<?php echo base_url('index.php/bms_property/add_resi_notice/');?>'+item.resident_notice_id+'" data-value="'+item.resident_notice_id+'" title="View"><i class="fa fa-edit"></i></a>';
                    str += '</td>';
                    str += '</tr>';
                });
            ...
            $('.notice_det_cls').bind("click",function () {
                $('.modal-body2').load('<?php echo base_url('index.php/bms_property/resident_notice_details/');?>'+$(this).attr('data-value'),function(result){
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_property/resi_notice_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

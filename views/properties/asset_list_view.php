              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_asset_btn" value="Add Asset" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_asset_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_property/add_asset');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_property/get_asset_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.asset_make+'</td>';
                    str += '<td>'+item.serial_no+'</td>';
                    str += '<td>'+item.asset_location+'</td>';
                    str += '<td>'+item.supplier_name+'</td>';                    
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_property/add_asset');?>/'+item.asset_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_property/property_asset_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

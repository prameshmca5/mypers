              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-md-4 col-xs-5">                        
                          <input type="text" id="search_txt" value="<?php echo isset($_GET['search_txt']) && trim($_GET['search_txt']) != '' ? trim($_GET['search_txt']) : '';?>" class="form-control" placeholder="Enter text to search Annual Renewal">
                    </div>
                    ...
                    <div class="col-md-3 col-xs-12">
                        <input class="btn btn-primary add_annual_renewal_btn" value="Add Annual Renewal" type="button">
                    </div>
...
$(document).ready(function () {
    ...
    $('.add_annual_renewal_btn').click(function () {
        window.location.href='<?php echo base_url('index.php/bms_property/add_annual_renewal');?>?property_id='+$("#property_id").val();
        return false;  
    });
...
function loadContent (offset,rows,flag) {
    ...
    $.ajax({
        type:"post",
        async: true,
        url: '<?php echo base_url('index.php/bms_property/get_annual_renewal_list');?>',
        ...
        success: function(data) {  
            ...
            if(numFound > 0) {                    
                $.each(data.records,function (i, item) { 
                    ...
                    str += '<td>'+item.item_descrip+'</td>';                    
                    str += '<td>'+item.serial_no+'</td>';
                    str += '<td>'+item.location+'</td>';
                    str += '<td>'+item.license_no+'</td>';
                    str += '<td>'+item.license_start_date+'</td>';
                    str += '<td>'+item.license_expiry_date+'</td>';
                    str += '<td>'+item.supplier_name+'</td>';                    
                    str += '<td style="text-align: center;"><a href="<?php echo base_url('index.php/bms_property/add_annual_renewal');?>/'+item.annual_renewal_id+'" title="Edit"><i class="fa fa-edit"></i></a></td>';
                    str += '</tr>';
                });
            ...
            if(flag) {
                if (typeof (history.pushState) != "undefined") {
                    var update_url = '<?php echo base_url('index.php/bms_property/annual_renewal_list');?>/'+offset+'/'+rows+'?property_id='+$('#property_id').val()+'&search_txt='+search_txt;

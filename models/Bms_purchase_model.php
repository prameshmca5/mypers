...
Class Bms_Purchase_model extends CI_Model {
    ...
    function chargecode() {
        $sql = "SELECT charge_code_group_id,charge_code_group_name FROM bms_charge_code_cat_group  ORDER by charge_code_group_name ASC";
        $query = $this->db->query($sql);
        
        return $query->result_array();
    }
     function sub() {
        $sql = "SELECT charge_code_category_id,charge_code_group_id,charge_code_category_name FROM bms_charge_code_category  ORDER by charge_code_category_name ASC";
        $query = $this->db->query($sql);
   ...
   public function insertmore($data = array()){
    $insert = $this->db->insert_batch('bms_purchase_orders',$data);
    return $insert?true:false;
  }
  
   public function record_purchase() {
        return $this->db->count_all("bms_purchase_orders");
    }
    public function fetch_purchase($limit, $start) {
        $this->db->limit($limit, $start);
        $query = $this->db->get("bms_purchase_orders");
  ...
  function getCommonDocs76 () {
	 ...
	 function insert_unit ($data){
        ...
        $this->db->insert('bms_bank_details', $data);
	...
	// print_r($nam);exit;
	 ...
	 function getCommonDocs($search_txt) {
        ...
        if($search_txt != '') {
            ...
            $cond = " AND (LOWER(a.bank_name) LIKE '%".$search_txt."%' OR LOWER(a.city) LIKE '%".$search_txt."%')";
        }
        $sql = "SELECT id,bank_name,ifc_code,branch_address,branch_number,state,city 
                FROM bms_bank_details AS a
                WHERE 1=1 $cond
                ORDER BY bank_name";
        $query = $this->db->query($sql);

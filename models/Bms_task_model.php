...
Class Bms_task_model extends CI_Model {
    ...
    function get_task ($type = '', $desi_id ='', $staff_id = '', $offset = '', $per_page = '',$property_id='',$task_status='',$task_sear='') {
        $limit = '';
        if($offset != '' && $per_page != '')
        ...
        if($task_sear != '')
            $cond2 .= ' AND a.task_id=\''.ltrim($task_sear,'0').'\' ';
        $sql = "SELECT task_id,property_name,task_name,due_date,a.created_date,task_status 
                ...
                AND a.property_id IN (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id.") ORDER BY task_status DESC, due_date ASC".$limit;
        $query = $this->db->query($sql);
        //echo "<br />".$this->db->last_query();
        return $query->result_array();
    }
    
    function get_task_with_num_rows ($type = '', $desi_id ='', $staff_id = '', $offset = '', $per_page = '',$property_id='',$task_status='', $task_sear='') {
        $limit = '';
        if($offset != '' && $per_page != '')
            $limit = ' LIMIT '. $offset .', '.$per_page;
        $cond = $type == '' || $type == 'own' ? '=' : '<>';
        ...
        $sql = "SELECT task_id,property_name,task_name,due_date,a.created_date,task_status 
                ...
                WHERE a.assign_to". $cond . $desi_id ." ".$cond2." AND 
                a.property_id IN (SELECT property_id FROM bms_staff_property WHERE staff_id=".$staff_id.") 
                ORDER BY task_status DESC, due_date ASC".$limit;
        $query = $this->db->query($sql);
        //echo "<br />".$this->db->last_query();
    ...
    function get_task_with_num_rows_jmb ( $property_id, $task_status='', $task_sear='', $offset = '', $per_page = '') {
        $limit = '';
        if($offset != '' && $per_page != '')
            $limit = ' LIMIT '. $offset .', '.$per_page;
        ...
        $sql = "SELECT task_id,property_name,task_name,due_date,a.created_date,task_status 
                    FROM bms_task a 
                    LEFT JOIN bms_property b ON b.property_id = a.property_id  
                    WHERE a.property_id = ".$property_id. $cond2. " ORDER BY task_status DESC, due_date ASC".$limit;
        $query = $this->db->query($sql);
        //echo "<br />".$this->db->last_query();
        $data = $query->result_array();
    ...
    function get_task_details ($task_id,$staff_id) {
        ...
        $sql = "SELECT task_id,task_name,due_date,a.created_date,task_status,task_location,task_details,task_category,task_source,task_update,assign_to,
                ...
                a.created_by,first_name,last_name, task_close_remarks
                FROM bms_task a 
                LEFT JOIN bms_property b ON b.property_id = a.property_id 
    ...
    function get_task_details_for_email ($task_id) {
        ...
        $sql = "SELECT task_id,task_name,b.property_name, d.owner_name, d.email_addr, d.gender
                FROM bms_task a 
                LEFT JOIN bms_property b ON b.property_id = a.property_id
                LEFT JOIN bms_property_units d ON d.unit_id = a.unit_id                
    ...
    function set_task_update_with_log ($task_id,$data,$staff_id) {
        ...
        if(isset($data['task_status']) && $data['task_status'] == 'C') { 
            ...
            $log_data['log_remarks'] = $data['task_update'];
        }
    ...
    function task_alert_insert ($task_id,$property_id,$alert_info) {
        ...
        $sub_sql = "SELECT '".$task_id."', '".$alert_info."', staff_id, '1', '".$data['created_date']."' FROM bms_staff_property where property_id = ".$property_id .$staff_sql;

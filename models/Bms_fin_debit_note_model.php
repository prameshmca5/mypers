...
Class Bms_fin_debit_note_model extends CI_Model {
    ...
    function get_unit_receipt ($unit_id,$receipt_id='') {
        $sql = "SELECT receipt_id,receipt_no,paid_amount                
                FROM bms_fin_receipt a                                
                WHERE unit_id =".$unit_id." 
                AND receipt_id NOT IN (SELECT receipt_id FROM bms_fin_debit_note WHERE unit_id=".$unit_id.")";
        if(!empty($receipt_id)) 
    ...
    function deleteDebitNote ($debit_note_id) {
        return $this->db->delete('bms_fin_debit_note',array('debit_note_id'=>$debit_note_id));
    }

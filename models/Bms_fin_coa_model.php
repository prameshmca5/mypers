...
Class Bms_fin_coa_model extends CI_Model {
    ...
    function get_coa_details($coa_id) { 
        $sql = "SELECT coa_id,property_id,coa_code,coa_name,coa_type_id,period,opening_debit,opening_credit,
                payment_source,payment_enabled,bill_enabled,receipt_enabled
                FROM  bms_fin_coa a               
                WHERE coa_id=". $coa_id;        
        $query = $this->db->query($sql);
        //echo "<br />".$this->db->last_query();
